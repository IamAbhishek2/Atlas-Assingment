...
//For Memory management used a little hack to just RUN the programme.
//Unable to check for the end cases where the command line will fail (Time constraint ) and half of things I have to figure out from error to make every separate function work.
...
//Trim the null space while encountering the problem when processing the mycat hello.txt then it is only taking the mycat and space.
...
int main() {
	...
	//Used malloc to process the getline 
	
	cmd = (char*)malloc(DEFAULT_BUFFER_SIZE);

	pipe(pipe_buf);
	
	printf(" To use this AbDutta Shell, Start with help command to know further\n ");
		
	printf("> ");

	getline(&cmd, (size_t*)&buf_size, stdin);
	
	//Token concept came into mind, when I want to make similar shell like the one we have for linux.
	//So basically for every command and it's arguement that I am passing, making it process in a single way, and then passing the output either to pipe or just printing it in console.
	...
	while(strcmp(trim(token), "exit")) {
		while(token) {
			...
			// Used PIPE in the main function for generating all the required command in Child process
			pid = fork();
			if (pid == 0) {
				if(!strcmp(cmd, "mycp")) 
